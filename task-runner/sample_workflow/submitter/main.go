...
func main() {
	...
	jobsClient := clientFromConfig(config.serverSocket, config.authToken)
	jobs := services.NewJobService(jobsClient)

	job, err := jobs.Submit(ctx, "spawn-workflow-tree", services.DefaultClusterSlug,
	...
	slog.InfoContext(ctx, "Job submitted", "job_id", uuid.Must(uuid.FromBytes(job.GetId().Uuid)))
}
...
// clientFromConfig returns a job service client based on the CLI flags
...
// This function is deliberately kept here directly in main, and a similar one in the runner/main.go file
// to keep those as examples as standalone as possible.
// That's also the reason we don't use our config package here.
func clientFromConfig(serverSocket, authToken string) workflowsv1connect.JobServiceClient {
	...
	slog.Info("Initiating job client for unix socket", "workflows-service-socket", serverSocket)
	return workflowsv1connect.NewJobServiceClient(client, "http://workflows-service") // URL here is irrelevant
}
